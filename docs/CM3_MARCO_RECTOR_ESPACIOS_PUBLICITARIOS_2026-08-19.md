...
- La integración OCU26 + YPF quedó completada.
- Fuente canónica: `input/OCU26_BASE_DATOS.xlsx`
- MAESTRO_ELEMENTOS: 5.304
- CAMPANAS: 15.333
- PARAMETROS: 23
- Catálogo YPF validado: 603 estaciones y 3.883 elementos.
- TV1–TV6 fueron regeneradas y continúan siendo el baseline funcional.
- Suite: 441 pruebas aprobadas y 5 fallas preexistentes de TV5 documentadas.
- Documento operativo de referencia:
  `docs/CM2_CIERRE_BASE_OCU26_YPF_2026-08-18.md`

## REGLA DE PRECEDENCIA

- Para datos, hashes, conteos, archivos y estado operativo prevalece CM2.
- Para la evolución conceptual de los dashboards prevalece el Marco Rector de Espacios de este documento.
- El modelo de Espacios Publicitarios todavía NO está implementado.
- Los valores anteriores de 451 estaciones, 305 activas y cualquier cifra del mockup son referencias históricas/no canónicas.
- El próximo paso será una auditoría consolidada READ-ONLY sobre la nueva base; no modificar todavía builders, semántica ni TVs.

# CONTEXTO MAESTRO DEL PROYECTO

**Proyecto:** BRAND PLUS · OCU26 · Sistema de inteligencia comercial, ocupación y dashboards  
**Estado documentado:** 18 de agosto de 2026, aproximadamente 17:31 ART
**Repositorio principal:** `C:\brand plus\ocu26-dashboard`  
**Branch vigente:** `main`  
**Fuente local vigente:** `input/OCU26_BASE_DATOS.xlsx`  
**Estado de entrega:** las seis TVs productivas preexistentes siguen funcionando como baseline; se acaba de cerrar un NUEVO MARCO RECTOR conceptual para migrar progresivamente toda la lectura comercial desde `elementos` hacia `ESPACIOS PUBLICITARIOS`. Esta reformulación todavía NO debe interpretarse como implementada en código ni como números recalculados.
**Prioridad inmediata después de esta migración:** Claude está terminando una actualización de base. Al cerrarla, informar el nuevo Marco Rector de Espacios mediante una auditoría READ-ONLY/semántica; luego definir cada TV conceptualmente y recién después implementar TV por TV. No modificar las seis TVs de golpe.
**Finalidad:** que un nuevo ChatGPT/Claude Code pueda continuar desde el estado exacto actual sin releer este chat.

> **REGLA DE VIGENCIA ABSOLUTA — 18/8/2026 ~17:31 ART:** la sección de actualización inmediata que sigue es la capa de vigencia MÁS RECIENTE. Si contradice cualquier regla, KPI, wording o lectura anterior basada en `elementos`, prevalece el nuevo **Marco Rector de Espacios Publicitarios**. Los dashboards actuales siguen siendo el baseline funcional/visual hasta que cada TV sea migrada de forma controlada. No interpretar los números de espacios del mockup conceptual como datos reales hasta auditar la base actualizada.

---


# ACTUALIZACIÓN VIGENTE CM1 — 18/8/2026 ~17:31 ART
## NUEVO MARCO RECTOR OCU26 — ESPACIOS PUBLICITARIOS

> **ESTA ES LA CAPA DE VIGENCIA MÁS RECIENTE DEL PROYECTO.**
>
> El cambio es conceptual y estructural: Brand Plus vende **espacios publicitarios**. Desde ahora, `ESPACIO PUBLICITARIO` pasa a ser la variable rectora transversal de OCU26 para Estático y Digital.
>
> Los `ElementoID`, soportes físicos, estaciones, campañas, activaciones, marcas, clientes, agencias, etc. NO desaparecen. Cambia su jerarquía: pasan a describir, generar, ocupar o explicar espacios.
>
> **IMPORTANTE:** este marco está cerrado conceptualmente, pero todavía debe auditarse contra la base recién actualizada antes de modificar builders, outputs o TVs. No multiplicar cifras de forma automática ni convertir un soporte activo en todos sus slots ocupados.

---

### MR.1 Decisión central confirmada

Pregunta madre del sistema:

**¿Cuántos espacios publicitarios tenemos, cuántos estamos utilizando y cómo se distribuyen?**

Esta pregunta debe organizar progresivamente toda la lectura ejecutiva del negocio.

Cadena conceptual:

`Elemento / soporte / estación`
→ `capacidad de espacios`
→ `espacios ocupados`
→ `espacios disponibles`
→ `% ocupación`
→ `campaña / marca / cliente / agencia / programática / ubicación / circuito / formato`

#### Consecuencia

- El negocio se explica y compara en **espacios**.
- El soporte físico sigue siendo necesario para saber cuánta capacidad comercial genera.
- `ElementoID` sigue siendo identificador técnico/físico.
- NO renombrar ni eliminar `ElementoID`.
- NO romper Gates ni relaciones existentes.
- La solución debe extender la capa semántica, no reconstruir el proyecto.

---

### MR.2 Cuatro métricas rectoras

#### 1. ESPACIOS TOTALES

Capacidad comercial total disponible para vender.

#### 2. ESPACIOS OCUPADOS

Espacios efectivamente utilizados/vendidos en el período.
...
Estas cuatro métricas pasan a ser el eje transversal.

---

### MR.3 Regla crítica: capacidad NO es ocupación

No confundir capacidad de un soporte con espacios efectivamente vendidos.

Ejemplo confirmado:

- 1 Pantalla LED = 20 slots/espacios de capacidad.
- Si tiene solo 3 slots ocupados:
  - Espacios totales = 20
  - Espacios ocupados = 3
...
`pantalla con campaña × 20`

NO equivale automáticamente a:

`20 espacios ocupados`.

La misma regla aplica a Tótems, Puentes LED y YPF.

---

### MR.4 Conversión confirmada soporte físico → espacios

| Familia / soporte | Unidad física / territorial | Capacidad comercial confirmada |
|---|---:|---:|
| Estático | 1 soporte | **1 espacio** |
| Pantalla LED | 1 pantalla | **20 espacios/slots por reel** |
| Tótem Digital de Shopping | 1 tótem | **10 espacios/slots por reel** |
| Puente LED | 1 puente | **10 espacios/slots por reel** |
| YPF | 1 estación | **5 espacios por estación** |

#### Ejemplo rector

`1 Pantalla LED + 1 cartel estático = 20 + 1 = 21 espacios publicitarios vendibles`

Físicamente son 2 soportes; comercialmente son 21 espacios.

---

### MR.5 Caso especial YPF — doble unidad

YPF debe conservar simultáneamente dos niveles.

#### Nivel territorial / físico

`1 estación = 1 estación`

Se mantienen como métricas válidas:

- estaciones totales;
- estaciones activas;
- estaciones con campaña;
- penetración de estaciones;
- cobertura territorial;
- mapa;
- ranking territorial.

Ejemplo actual pre-migración:

- 451 estaciones en catálogo;
- 305 estaciones activas en julio.

305 activas siguen siendo **305 estaciones**, no 1.525 estaciones.

#### Nivel comercial

`1 estación = 5 espacios publicitarios`

Capacidad total teórica actual:

`451 × 5 = 2.255 espacios YPF`

Pero:

`305 estaciones activas × 5 = 1.525`

representa la **capacidad potencial de espacios asociada a esas 305 estaciones**, NO necesariamente 1.525 espacios ocupados.

Para llamar a un espacio YPF “ocupado” debe existir evidencia en la base que permita identificar cuántos de los 5 slots de la estación están efectivamente utilizados.

Si la granularidad actual no permite saberlo:

- conservar estaciones activas;
- conservar capacidad comercial = 5 por estación;
- NO inventar espacios ocupados;
- usar `PARTIAL`, `REQUIERE_CONFIRMACION` o MetricStatus canónico equivalente.

---

### MR.6 Activaciones / repeticiones vs espacios ocupados

La nueva lectura busca saber **cuántos espacios tienen campañas**, no cuántos elementos tienen campañas.

Pero no asumir automáticamente:

`activación = espacio ocupado`

hasta reconciliar el grano temporal.

Una misma campaña puede repetirse en varios espacios y un mismo espacio puede ser utilizado por distintas campañas en momentos diferentes.

Antes de convertir activaciones/repeticiones en ocupación canónica se debe definir:
...
- soporte;
- campaña;
- si varias campañas pueden ocupar el mismo espacio en el mismo reel/período;
- regla de deduplicación.

Estado: **A VALIDAR contra la base actualizada**.

---

### MR.7 Jerarquía de variables desde ahora

#### Variable rectora

**ESPACIO PUBLICITARIO**

#### Variables físicas / de inventario
...
#### Variables de demanda / comercialización
...
Todas deben terminar explicando:

- quién ocupa espacios;
- cuántos;
- dónde;
- mediante qué canal/relación;
- con qué nivel de ocupación.

---

### MR.8 TV1 — conceptual vigente antes de implementación

TV1 pasa a responder prioritariamente:

**¿Cuántos espacios publicitarios tenemos, cuántos estamos utilizando y cómo se distribuyen?**

Los elementos físicos pueden seguir apareciendo como dato secundario de equivalencia.

#### Cambios conceptuales solicitados visualmente sobre la captura actual

El usuario marcó explícitamente:

1. `(x: 10,5%, y: 20,2%)`
   - cambiar lectura de elementos a **espacios**;
   - sumar el Core Comercial: Shoppings + Pantallas LED + AA2000 + YPF.

2. `(x: 13,3%, y: 32,9%)`
   - debajo del número de espacios mostrar, en menor jerarquía, **a cuántos elementos/soportes físicos corresponde**.

3. `(x: 29,5%, y: 20,2%)`
   - mantener conceptualmente la tarjeta actual.

4. `(x: 40,8%, y: 21,7%)`
   - reformular como activaciones/repeticiones o, preferentemente, **cuántos espacios tienen campañas**;
   - NO elementos;
   - mantener comparación temporal pero expresada en espacios;
   - definición final depende de auditoría de grano.

5. `(x: 59,1%, y: 17,9%)`
   - mantener la lógica salvo que la unidad principal pase de elementos a **espacios**.

6. `(x: 71,1%, y: 20,7%)`
   - pasar de elementos a **espacios digitales / slots**.

7. `(x: 90,1%, y: 21,5%)`
   - mantener lectura de **espacios por calendario**.

8. `(x: 27,8%, y: 47,5%)`
   - el histórico/evolución debe hablar de **espacios**, no elementos.

#### Muy importante sobre el mockup conceptual

Se generó un mockup visual para representar esta dirección.

Los números de espacios que aparecieron en ese mockup, por ejemplo:

- `4.235`
- `1.525`
- `1.065`
- cualquier serie histórica derivada por multiplicación

son **NO CANÓNICOS / A VALIDAR**.

Sirven como demostración visual de la nueva semántica, NO como cálculo aprobado.

No hardcodear esos valores.

---

### MR.9 Relectura conceptual de las seis TVs — aprobada
...
Elementos/estaciones pueden aparecer como equivalencias secundarias.

#### TV2 — Core Comercial Digital

Pregunta:

**¿Cómo se ocupan y monetizan los espacios digitales?**

Debe evolucionar hacia:
- capacidad total de slots;
- slots ocupados;
- slots disponibles;
- fill/% ocupación;
- capacidad y performance por tipo de soporte;
- cobertura física como dato secundario.

Pantallas LED = 20.
Tótems = 10.
Puentes LED = 10.
AA2000 debe auditarse antes de asignar capacidad.

#### TV3 — Core Comercial Estático

Pregunta:

**¿Cómo se ocupan y dónde quedan disponibles los espacios estáticos?**

Conversión directa:

`1 soporte estático = 1 espacio`

Por lo tanto:
- ocupados = soportes estáticos ocupados;
- disponibles = soportes libres;
- ocupación = ocupados / total.

La palabra ejecutiva debe ser **espacios** aunque `ElementoID` siga existiendo técnicamente.

#### TV4 — Pipeline Comercial

Pregunta futura:

**¿Qué espacios están comprometidos hoy y cuáles entran o se liberan próximamente?**

Campañas únicas continúan siendo relevantes.

Pero deben empezar a explicar:
- espacios comprometidos;
- espacios reservados;
- espacios que inician;
- espacios que se liberan;
- capacidad futura.

No convertir las 421 activaciones actuales automáticamente en 421 espacios sin validar grano y unicidad.

#### TV5 — YPF

Debe mantener dos perspectivas:
...
**Capacidad/ocupación comercial**
- 5 espacios por estación;
- espacios YPF totales;
- espacios ocupados solo si se pueden conocer realmente;
- disponibilidad/ocupación cuando el dato sea soportado.

La estación sigue siendo la variable territorial principal.

#### TV6 — Demanda Comercial

Pregunta:

**¿Quién está comprando nuestros espacios, a través de qué relación comercial y dónde los concentra?**

Reformular las dimensiones:

**Marca**
= qué marca/anunciante utiliza espacios.

**Cliente directo**
= compra directa cuando los datos permiten afirmarlo.

**Agencia**
= intermediario/agencia asociada a la compra.

**Programática**
= modalidad/canal de comercialización; NO es sinónimo de agencia.

La lectura futura puede separar:

`MARCAS → CLIENTES DIRECTOS → AGENCIAS → PROGRAMÁTICA`

Todos deben poder explicarse finalmente por **espacios ocupados**.

No comparar marca y agencia como si fueran la misma dimensión.

---

### MR.10 Qué permanece vigente de los dashboards actuales

Hasta que cada TV sea migrada:

- `tv1.html` … `tv6.html` siguen siendo los productos funcionales de baseline;
- no destruirlos;
- no cambiar todas las TVs en bloque;
- las métricas anteriores de elementos/activaciones quedan como referencia pre-migración;
- los tests anteriores prueban el baseline anterior, NO el nuevo modelo de espacios;
- el nuevo marco deberá tener tests propios cuando se implemente.

---

### MR.11 Estado operativo exacto al 18/8/2026 ~17:31 ART

Claude Code está terminando **otra actualización de la base**.

No iniciar todavía una implementación de Espacios mientras esa actualización no se confirme cerrada.

Cuando termine:

1. verificar brevemente que la actualización de base quedó finalizada;
2. informar a Claude el Marco Rector de Espacios;
3. pedir una **única auditoría consolidada READ-ONLY**;
4. determinar qué espacios pueden calcularse exactamente;
5. diferenciar exacto / parcial / no disponible;
6. NO modificar TVs en esa primera auditoría;
7. después revisar conceptualmente TV por TV;
8. recién entonces implementar.

---

### MR.12 Auditoría inicial requerida a Claude después de cerrar la base

La auditoría debe responder, por familia:

`Familia | Unidad física | Regla espacios | Capacidad calculable | Ocupación calculable | Fuente | MetricStatus | Observación`
...
- YPF;
- AA2000;
- cualquier otro soporte real del Core.

Debe separar:

#### Exactamente calculable
La base identifica capacidad y ocupación sin inferencias.

#### Parcialmente calculable
Capacidad conocida pero ocupación no completamente observable.

#### No disponible
La base no permite construir la métrica sin nueva información/regla.

No modificar en esa auditoría:
...
### MR.13 Propuesta semántica — todavía NO implementar
...
Los nombres son referenciales.

Claude debe revisar si la arquitectura actual ya dispone de campos/métricas equivalentes.

No crear taxonomía paralela innecesaria.

---

### MR.14 Orden de trabajo

Decisión confirmada:

**primero conceptual/estructural; después implementación.**

No modificar las seis TVs de una vez.

TV1 ya tiene una primera definición conceptual de cambios.

Luego se detallará cada TV junto con el usuario.

Orden sugerido de análisis por dependencia del modelo:

1. TV1 — visión global;
2. TV2 — capacidad Digital/slots;
3. TV3 — Estático 1:1;
4. TV5 — YPF estación + espacios;
5. TV4 — Pipeline en espacios;
6. TV6 — demanda por espacios.

Este orden es operativo/sugerido; si el usuario cambia la prioridad de una TV, prevalece la instrucción posterior.

---

### MR.15 Regla para futuras sesiones de Claude

Para cada TV:

- preferir su sesión específica de Claude Code;
- usar `docs/CM1.md` como fuente global;
- pedir lectura focalizada;
- no releer todo el repo;
- no reauditar métricas ya cerradas salvo impacto directo del nuevo marco;
- prompt maestro por TV;
...
Durante retoques estéticos:

`Solo ajustes estéticos; lógica y métricas congeladas salvo error evidente. No tocar Gates ni otras TVs.`

---

### MR.16 Nuevos A VALIDAR introducidos por el Marco Rector

1. Cantidad exacta de espacios totales del Core después de la base actualizada.
2. Ocupación real de slots de cada Pantalla LED.
3. Ocupación real de Tótems Digitales.
4. Ocupación real de Puentes LED.
5. Capacidad exacta de soportes AA2000.
6. Cómo identificar cada uno de los 5 espacios YPF, si existe esa granularidad.
7. Ocupación real YPF en espacios.
8. Regla temporal/unicidad para transformar activaciones/repeticiones en espacios ocupados.
9. Reconciliación histórica Ene–Jul en espacios.
10. Nuevo denominador de Core en espacios.
11. Relectura de comparativas mensuales en espacios.
12. Relectura de YTD en espacios.
13. TV6: clasificación canónica Cliente Directo vs Agencia.
14. TV6: Programática como canal/modalidad, no como agencia.
15. Tests que deberán agregarse o modificarse cuando el modelo se implemente.

---

### MR.17 Reglas que NO deben perderse durante la transición

- APSA y London siguen excluidos.
- YPF no debe recibir un fill inventado.
- unknown ≠ 0.
- `MetricStatus` sigue vigente.
- no usar valores dummy/legacy.
- `ElementoID` e `IDCampaña` siguen vigentes.
- no cambiar IDs ni Gates por la nueva lectura.
- no hardcodear multiplicaciones en HTML.
- HTML no se convierte en motor de negocio.
- la lógica de espacios debe vivir en la capa semántica/canónica apropiada.
- capacidad ≠ ocupación.
- estación YPF ≠ espacio YPF.
- campaña ≠ activación ≠ espacio.
- marca ≠ cliente directo ≠ agencia ≠ programática.

---

### MR.18 Próximo paso exacto

**Esperar a que Claude termine la actualización actual de base.**

Luego:

1. confirmar cierre de esa actualización;
2. pasarle el prompt maestro del Marco Rector de Espacios;
3. hacer auditoría READ-ONLY;
4. traer el resultado a ChatGPT;
5. cerrar definiciones faltantes;
6. revisar TV1 conceptualmente cuadro por cuadro;
7. continuar TV por TV;
8. no implementar masivamente.
