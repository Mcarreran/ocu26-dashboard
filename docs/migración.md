...
**Estado documentado:** 18 de agosto de 2026, durante la preparación de la migración de computadora  
**Repositorio principal:** `C:\brand plus\ocu26-dashboard`  
**Branch vigente:** `main`  
**Fuente local vigente:** `input/OCU26_BASE_DATOS.xlsx`  
**Estado de entrega:** las seis TVs productivas existen, fueron publicadas manualmente en Netlify y tienen URLs públicas estables bajo `https://dashoperabp.netlify.app/`; TV5 conserva cartografía real de Natural Earth.  
**Prioridad inmediata después de esta migración:** completar y verificar la migración íntegra de `C:\brand plus\` a la computadora nueva antes de devolver la computadora actual. Después se retomarán el Form de carga, SharePoint y el ordenamiento de bases sin alterar los dashboards publicados. La operación de publicación permanece manual en la etapa actual. La conexión GitHub → Netlify y la automatización de regeneración/publicación quedan expresamente para la Etapa 2. Power BI sigue pausado.  
**Finalidad:** que un nuevo ChatGPT/Claude Code pueda continuar desde el estado exacto actual sin releer este chat.

> **REGLA DE VIGENCIA ABSOLUTA — 18/8/2026:** la actualización de máxima vigencia incorporada inmediatamente debajo de este aviso prevalece sobre el snapshot del 13/8/2026 y sobre los anexos históricos. Las secciones 0–45 y los anexos se conservan completos para trazabilidad. Ante contradicción, usar siempre la decisión más reciente fechada. No usar estados viejos de publicación, dominios Netlify anteriores, numeraciones históricas de TVs, reglas YPF reemplazadas ni planes de automatización inmediata que contradigan esta capa.

## ACTUALIZACIÓN DE MÁXIMA VIGENCIA — 18/8/2026 — MIGRACIÓN DE PC, FORMULARIO Y ÚLTIMO CRUCE YPF

Esta sección registra los cambios materiales ocurridos después del cierre documentado el 13/8/2026. Es la primera capa que debe leer un nuevo ChatGPT o Claude Code. No elimina ni resume el contenido anterior: lo corrige y complementa.

### A. Punto exacto actual

Al momento de crear este archivo:

1. El proyecto productivo continúa físicamente en la computadora que debe devolverse, bajo `C:\brand plus\`.
2. El repositorio principal continúa en `C:\brand plus\ocu26-dashboard\`.
3. La base canónica local continúa en `C:\brand plus\ocu26-dashboard\input\OCU26_BASE_DATOS.xlsx`.
4. El paquete manual de publicación continúa en `C:\brand plus\OCU26_WEB\`.
5. Las seis TVs ya están publicadas en `https://dashoperabp.netlify.app/` y su funcionamiento no depende de que la PC permanezca encendida.
6. Se estaba esperando que Claude Code terminara un último cruce/auditoría de YPF antes de cerrar aplicaciones y comenzar la migración.
7. El usuario pidió avanzar paso a paso en este orden: carpetas de `C:` → GitHub → datos locales de Claude Code → restauración y prueba en la PC nueva.
8. El Form de carga todavía no debe implementarse ni conectarse a la base local hasta terminar y validar la migración.
9. El estado exacto del último proceso de Claude Code posterior a la última captura no está disponible en este chat: `A VALIDAR` antes de cerrar la PC actual.

### B. Decisión vigente de migración

Regla operativa: **nunca mover ni cortar; siempre copiar, verificar y conservar el original hasta que la restauración funcione.**

Respaldo principal a crear:

```text
C:\brand plus\
  ↓ comprimir con aplicaciones cerradas
BP_OCU26_MIGRACION_COMPLETA_2026-08-18.zip
  ↓ subir a almacenamiento privado
  ↓ descargar en la PC nueva
  ↓ verificar SHA-256
  ↓ extraer directamente en C:\
C:\brand plus\
```

La migración debe preservar como mínimo:

```text
C:\brand plus\ocu26-dashboard\
C:\brand plus\OCU26_WEB\
```

No debe quedar una ruta duplicada como:

```text
C:\brand plus\brand plus\ocu26-dashboard\
```

Antes de comprimir deben cerrarse Claude Code, Excel, Power BI, VS Code, GitHub Desktop, previews y cualquier proceso que pueda estar escribiendo archivos.

Verificación del ZIP original en PowerShell:

```powershell
Get-FileHash -Algorithm SHA256 "C:\BP_OCU26_MIGRACION_COMPLETA_2026-08-18.zip"
```

La ruta debe adaptarse si el ZIP se crea en otra ubicación. La huella SHA-256 debe guardarse y compararse con la del archivo descargado en la PC nueva. También debe compararse el tamaño y abrirse el ZIP para confirmar que contiene la jerarquía esperada.

La PC anterior no debe devolverse hasta confirmar en la nueva:

- que abre `OCU26_BASE_DATOS.xlsx`;
- que existen `tv1.html` a `tv6.html`;
- que existen `input`, `input_aux`, `config`, `scripts`, `tests`, `output`, `audit_sources`, `powerbi`, `.git` y los documentos/contextos del proyecto;
- que existe `OCU26_WEB` con `index.html`, seis HTML, seis JSON y los GeoJSON;
- que GitHub Desktop reconoce el repositorio;
- que Claude Code abre desde `C:\brand plus\ocu26-dashboard` y reconoce las instrucciones del proyecto;
- que una validación segura del pipeline puede ejecutarse sin modificar la fuente.

### C. GitHub: qué está en la cuenta y qué no

GitHub conserva solamente archivos versionados, commits realizados y commits enviados al remoto. No sustituye el ZIP completo.

Secuencia vigente después de crear y subir el ZIP:

1. abrir GitHub Desktop;
2. seleccionar `ocu26-dashboard`;
3. confirmar branch `main`;
4. ejecutar `Fetch origin`;
5. revisar `Changes`;
6. si hay cambios, no descartarlos y revisarlos antes de hacer commit;
7. si aparece `Push origin`, enviar los commits locales;
8. comprobar en GitHub web que el último commit y su fecha estén visibles;
9. conservar igualmente el ZIP, porque Git puede ignorar Excel, outputs, entornos u otros archivos.

No ejecutar `git add .`, commit, push, deploy ni descartes destructivos sin revisar el alcance. La regla histórica de aprobación explícita para Git sigue vigente.

### D. Claude y Claude Code: qué está en la cuenta y qué es local

Los chats normales asociados a la cuenta de Claude deberían reaparecer al iniciar sesión con la misma cuenta. Sin embargo, archivos del proyecto, sesiones locales, configuración, permisos, memorias locales y conexiones de Claude Code pueden estar fuera de la nube.

Debe respaldarse además:

```text
%USERPROFILE%\.claude
%USERPROFILE%\.claude.json   (si existe)
```

Nombre previsto:

```text
CLAUDE_CODE_LOCAL_2026-08-18.zip
```

Dentro del repositorio también deben preservarse, si existen:

```text
C:\brand plus\ocu26-dashboard\.claude\
C:\brand plus\ocu26-dashboard\CLAUDE.md
```

El ZIP de Claude puede contener información privada o configuración sensible. Debe mantenerse en almacenamiento privado y no publicarse ni incluirse en Netlify/GitHub público.

En la PC nueva, Claude Code debe instalarse e iniciar sesión normalmente; con Claude Code cerrado se restaura la carpeta `.claude` en el perfil nuevo. Algunas credenciales o permisos pueden requerir autenticación renovada y no deben copiarse o forzarse si el sistema los rechaza.

### E. Arquitectura vigente del Form de carga después de reconocer que OCU26 está en `C:`

La arquitectura anterior `Microsoft Forms → Power Automate → Excel maestro SharePoint` se corrigió porque la fuente productiva real seguía local. La arquitectura vigente para la primera etapa es:
...
1 flujo Power Automate
        ↓
OCU26_ENTRADAS_FORM.xlsx en SharePoint
        ↓
tabla tblEntradas / bandeja de entrada
        ↓
importación local controlada y validada
        ↓
C:\brand plus\ocu26-dashboard\input\OCU26_BASE_DATOS.xlsx
        ↓
Gates + pipeline central + builders
        ↓
TV1–TV6 / JSON / OCU26_WEB
```

`OCU26_ENTRADAS_FORM.xlsx` es una **bandeja de entrada**, no una segunda base maestra. Cada registro debe poder quedar con uno de estos estados:
...
La importación debe validar antes de escribir en la fuente canónica. No conectar Forms directamente con `OCU26_BASE_DATOS.xlsx` ni mantener dos copias maestras que evolucionen por separado.

Sitio SharePoint conocido:

```text
https://brandpluslatam.sharepoint.com/sites/OCU
```

Estructura productiva propuesta después de validar la nueva PC:
...
├── 02_ENTRADAS_FORM/
│   └── OCU26_ENTRADAS_FORM.xlsx
├── 03_HISTORICOS/
├── 04_DOCUMENTACION/
└── 05_POWER_BI/
```

Esta estructura es objetivo posterior a la migración. Mientras la restauración no esté validada, la fuente canónica sigue siendo `C:\brand plus\ocu26-dashboard\input\OCU26_BASE_DATOS.xlsx`.

No se eligió acceso directo de Power Automate al disco `C:` porque requeriría gateway y una PC disponible, agregando dependencia operativa contraria a la Etapa 1 simple. La sincronización total de `C:\brand plus` con SharePoint tampoco se recomienda: `.git`, `.venv`, outputs y archivos generados pueden causar conflictos. SharePoint debe almacenar datos/documentación controlados, no duplicar indiscriminadamente todo el repo.

### F. Regla semántica vigente para APSA, London Supply, Cencomedia y MAB

La frase histórica “APSA y London Supply excluidos” debe interpretarse con precisión:

- Para los dashboards de performance Core y sus numeradores, denominadores, rankings y comparativos estándar, se excluyen por defecto `London Supply`, `APSA`, `Cencomedia` y `MAB`, salvo una vista que explícitamente los incluya.
- `APSA` es el único portfolio `LEGACY` confirmado en la capa semántica vigente.
- `London Supply` debe clasificarse como `CONSULTA` en modo de disponibilidad y como `COMPLEMENTARIO` en portfolio; no debe tratarse como autoservicio calculable solo porque existan capacidades cargadas.
- `Cencomedia` puede entrar en catálogo ampliado o análisis estático cuando corresponda, pero no se fuerza dentro del Core ni se inventa capacidad.
- `MAB` no debe incorporarse al Core por inercia; su clasificación final continúa sujeta a la semántica vigente y a validación cuando una vista lo necesite.
- Los HTML productivos ya cerrados no deben reabrirse únicamente por esta aclaración. La aplicación concreta se verifica en una reconciliación semántica controlada.

Esto reemplaza cualquier lectura simplificada que convierta a London Supply en `LEGACY` junto con APSA o que incluya automáticamente Cencomedia/MAB en performance Core.

### G. Último trabajo separado de YPF — auditoría y normalización

Este trabajo se realizó como frente separado dentro del mismo proyecto. Su objetivo es unificar YPF digital, estático, estaciones, elementos y campañas sin modificar silenciosamente la fuente OCU26 existente hasta completar validaciones.

Archivos y hojas mencionados:

- `YPF DIGITAL + ESTATICO (1).xlsx`;
- hoja `DIGITAL`;
- hoja `ESTATICO `, cuyo nombre contiene un espacio final y debe respetarse en scripts;
- hojas/zonas de control `ZONAS` y `DIRECCIONES`;
- `YPF - Base campañas y elementos corregido estructural.xlsx`;
- hojas `LEEME`, `AUDITORIA_REV2`, `ALTAS_BASE_MAESTRA`.

Resultados de auditoría preservados:

- `DIGITAL`: 416 registros válidos; sumas de inventario `MB=437`, `TT=2142`, `PPUNTER=968`; total digital `3547`.
- `ESTATICO `: 191 registros válidos; `FB=383`.
- La fila 194 espuria de `ESTATICO ` tenía `APIE=None` y `FB=39`; fue excluida del resultado válido.
- Se revisaron APIE objetivo `{30510, 31131, 31192, 31239}` en `DIGITAL`, `ESTATICO ` y `DIRECCIONES`.
- En `ZONAS`/`DIRECCIONES` existen múltiples columnas con APIE, mencionadas en posiciones/columnas 2, 12, 23 y 34; se detectaron filas `TOTAL` y valores de control `386`, `404`, `2022`, `910`, `3336`.

Estructura objetivo:

- base de estaciones: APIE + dirección como clave alimentadora;
- base de elementos: una fila por elemento con `ElementoID` estable;
- separación de digital y estático;
- `FB` solo estático;
- eliminación de APIE vacío;
- control de duplicados por APIE/formato/correlativo;
- hojas de zonas/direcciones como control, no como sustituto no validado de la fuente.

Convención vigente de `ElementoID` YPF:

```text
APIE - FORMATO - CORRELATIVO
```
...
```regex
^\d+ - (MB|PPUNTER|TT|FB) - \d+$
```

Los IDs `100000` fueron colocados manualmente por el usuario durante la primera carga de YPF, cuando los registros todavía no estaban categorizados. Regla indicada por el usuario: si coinciden APIE y fechas con registros equivalentes ya cargados bajo el formato final, deben considerarse los mismos registros y reemplazarse por los IDs definitivos, no conservarse como campañas separadas.

Se mencionaron en las últimas capturas/revisiones `AID 30943`, “elementos digitales”, `4470` y `Beeyond`. El detalle exacto del problema y la resolución final no puede reconstruirse de forma segura con el texto disponible: `A VALIDAR` contra la salida final de Claude Code y las capturas originales antes de consolidar la base.

No alterar la base canónica OCU26 por inferencia. Primero debe cerrarse el último cruce, revisar el informe de Claude y validar altas/reemplazos/duplicados.

### H. Prioridades actualizadas

#### P0 — Crítico / bloqueante

1. Esperar y guardar el resultado del último cruce de Claude Code.
2. Cerrar aplicaciones y crear `BP_OCU26_MIGRACION_COMPLETA_2026-08-18.zip`.
3. Subir el ZIP completo a almacenamiento privado y registrar su SHA-256.
4. Revisar GitHub Desktop sin descartar cambios.
5. Respaldar `%USERPROFILE%\.claude` y `.claude.json` si existe.
6. Restaurar y verificar todo en la PC nueva antes de devolver la anterior.

#### P1 — Alta prioridad después de restaurar

1. Verificar `git status`, rutas, base, pipeline, HTML y `OCU26_WEB`.
2. Reconciliar el resultado final del cruce YPF y documentar `AID 30943`, los IDs `100000`, `4470/Beeyond` y elementos digitales.
3. Crear `OCU26_ENTRADAS_FORM.xlsx` y `tblEntradas` en SharePoint.
4. Diseñar Microsoft Forms con campos compatibles con la base vigente.
5. Crear un único flujo Power Automate que escriba en la bandeja de entrada.
6. Preparar un importador local controlado y probar una campaña completa.

#### P2 — Importante

1. Organizar SharePoint productivo sin convertirlo prematuramente en una segunda fuente maestra.
2. Retomar reconciliaciones transversales de TVs y Power BI.
3. Probar las URLs en el equipo real/CMS.

#### P3 — Futuro

1. Migrar de Excel de entrada a SharePoint Lists cuando la concurrencia/robustez lo justifique.
2. Automatizar regeneración y Netlify mediante una superficie pública mínima y segura.
3. Eliminar la dependencia operativa de una PC local solo cuando la fuente canónica y el runner estén resueltos.

### I. Próximo paso exacto vigente

No comenzar Forms todavía. El próximo paso exacto es:

1. esperar que Claude Code termine el último cruce;
2. guardar/exportar su resultado final;
3. cerrar Claude Code, Excel, VS Code, Power BI, GitHub Desktop y previews;
4. iniciar la Etapa 1 de migración copiando y comprimiendo `C:\brand plus\` sin mover ni borrar el original.

Cuando la PC nueva esté verificada, el siguiente frente será `Microsoft Forms → Power Automate → OCU26_ENTRADAS_FORM.xlsx → importación controlada`.

### J. Imágenes/capturas nuevas que deben volver a cargarse

| Imagen a recuperar | Qué contiene | Por qué es necesaria | Decisiones asociadas | Prioridad |
|---|---|---|---|---|
| Últimas capturas del cruce YPF en Claude Code | resultados de validación, conflictos, AID y elementos digitales | no hay texto suficiente para reconstruir la resolución | cierre de normalización YPF y migración segura | CRÍTICA |
| Captura donde aparece `AID 30943` | detalle del registro y problema particular | permite identificar el conflicto exacto | corrección o validación de campaña/elementos | CRÍTICA |
| Capturas donde aparecen IDs `100000` y sus coincidencias | registros iniciales no categorizados y matches por APIE/fechas | determina reemplazos y evita duplicados | sustitución por IDs finales | CRÍTICA |
| Captura/reporte relacionado con `4470` y `Beeyond` | contexto no recuperable con certeza | evita inventar el significado | auditoría final | IMPORTANTE |
| Captura final de GitHub Desktop antes de migrar | branch, Changes, Push origin | prueba qué cambios están solo locales | commit/push y respaldo | CRÍTICA |
| Captura o texto de SHA-256 de ambos ZIP | huellas de integridad | permite comparar descarga/restauración | cierre seguro de migración | CRÍTICA |

### K. Regla de vigencia entre esta actualización y el cuerpo anterior

Si el cuerpo anterior indica que el próximo paso es editar TVs, probar Netlify o iniciar SharePoint directamente, esta actualización lo reemplaza temporalmente: **primero migrar y verificar la computadora**. Si el cuerpo anterior presenta Forms escribiendo directamente en la base maestra, usar la bandeja `OCU26_ENTRADAS_FORM.xlsx`. Si trata a London Supply como `LEGACY`, usar `CONSULTA` + `COMPLEMENTARIO`; APSA conserva `LEGACY`. Si afirma que APIE no existe en ninguna fuente, distinguir la base OCU26 histórica de los archivos YPF nuevos que sí trabajan explícitamente con APIE.

---

## ACTUALIZACIÓN DE MÁXIMA VIGENCIA — PUBLICACIÓN Y ETAPA 2

Esta actualización registra el cierre operativo ocurrido el **13 de agosto de 2026** y reemplaza cualquier estado anterior que indicara `deploy pendiente`, que tratara `https://digitalcore-brandplus.netlify.app/` como URL objetivo vigente o que propusiera conectar GitHub de inmediato.

### Publicación terminada y funcionando

- Se creó manualmente una carpeta de publicación separada llamada `OCU26_WEB`.
- No se publicó el repositorio completo ni la base Excel.
- La carpeta se desplegó usando **Netlify Drop** desde una cuenta Netlify propia iniciada mediante autenticación con GitHub.
- El proyecto Netlify fue renombrado a `dashoperabp`.
- Dominio público vigente: `https://dashoperabp.netlify.app/`.
- La URL principal abre `index.html`, que en esta etapa es una copia de `tv1.html`.
- El sitio es público y no depende de que la computadora local permanezca encendida.

### URLs vigentes de las seis TVs

1. TV1: `https://dashoperabp.netlify.app/tv1.html`
2. TV2: `https://dashoperabp.netlify.app/tv2.html`
3. TV3: `https://dashoperabp.netlify.app/tv3.html`
4. TV4: `https://dashoperabp.netlify.app/tv4.html`
5. TV5: `https://dashoperabp.netlify.app/tv5.html`
6. TV6: `https://dashoperabp.netlify.app/tv6.html`

Portada/TV1: `https://dashoperabp.netlify.app/`

El dominio provisional `tiny-taffy-7dad45.netlify.app` dejó de ser la dirección operativa después del cambio de nombre. `https://digitalcore-brandplus.netlify.app/` pertenece a una prueba o cuenta anterior y **no es el proyecto administrado actualmente**.
...
Los HTML publicados son los archivos productivos de la raíz del repositorio, no los templates crudos. Los JSON se copiaron desde `C:\brand plus\ocu26-dashboard\output\`. Los GeoJSON conservaron la ruta relativa original para no romper TV5.

### Operación vigente — etapa actual manual

Cuando cambien los datos:

`base corregida → Gates/validaciones → builders TV1–TV6 → revisión de HTML productivos → actualizar OCU26_WEB → nuevo Netlify Drop`

Las URLs permanecen iguales al volver a desplegar en el mismo proyecto Netlify.

### Decisión final de automatización

La automatización se realizará en **Etapa 2**, no ahora. En la etapa actual no es necesario conectar GitHub desde el aviso de Netlify.

Objetivo de Etapa 2:

`fuente canónica actualizada → validaciones/Gates → builders → HTML/JSON/assets publicables → repositorio de publicación → Netlify automático → mismas URLs → CMS/TVs`

GitHub tendrá estas funciones:

- versionar los archivos y cambios;
- permitir recuperar versiones anteriores;
- alojar una superficie de publicación controlada;
- disparar el despliegue automático de Netlify al hacer push;
- eliminar la necesidad de arrastrar manualmente `OCU26_WEB` en cada actualización.

Antes de automatizar debe resolverse qué copia de la base es canónica y cómo llega desde SharePoint/Microsoft 365 al pipeline sin crear dos fuentes de verdad. No conectar el repositorio completo a Netlify sin revisar qué archivos quedarían públicos. Se recomienda una carpeta o repositorio de publicación que contenga únicamente los artefactos web necesarios.

### Validaciones todavía necesarias

- Confirmar las seis URLs en navegador.
- Confirmar que ninguna TV muestre `S/D` por fallo de carga.
- Confirmar particularmente que TV5 cargue mapa, provincias y puntos.
- Probar al menos una URL en el equipo real LG/webOS o CMS.
- Confirmar ausencia de scroll, recortes y problemas de tipografía/acentos.

Estas validaciones no reabren la lógica de métricas ni autorizan modificaciones de Gates.
...
- publicación manual Netlify completada en proyecto propio `dashoperabp`.
- seis URLs públicas estables bajo `https://dashoperabp.netlify.app/`.
...
- hosting Netlify sin necesidad de PC local encendida.
- portada `index.html` y rutas independientes `tv1.html` … `tv6.html`.
...
- Power BI;
- SharePoint;
- automatización GitHub → Netlify en Etapa 2.
...
16. prueba física de las URLs en TV/webOS/CMS.
17. Power BI `dataType`.
18. automatización de publicación en Etapa 2, después de resolver fuente canónica y superficie pública.
...
Se consideró como Plan B gratuito. Fue reemplazado para la operación actual por Netlify Drop manual. GitHub sigue siendo válido para versionado y automatización de Etapa 2, pero GitHub Pages no es el hosting productivo vigente.
...
### Publicación
Deploy manual confirmado en Netlify el 13/8/2026. Proyecto `dashoperabp`; seis URLs públicas vigentes. Falta prueba física TV/webOS/CMS.

### Automatizaciones
No configuradas. Quedan para Etapa 2 después de resolver fuente canónica y superficie pública mínima.
...
### Netlify
Hosting productivo actual. Proyecto `dashoperabp`, desplegado manualmente mediante Netlify Drop. GitHub todavía no conectado.
...
Publicación vigente confirmada el 13/8/2026:
- plataforma: Netlify;
- método actual: Netlify Drop manual;
- proyecto administrado: `dashoperabp`;
- URL: `https://dashoperabp.netlify.app/`;
- GitHub aún no está conectado a este proyecto Netlify;
- la conexión GitHub → Netlify se reserva para Etapa 2.

Referencia histórica no vigente:
- `https://digitalcore-brandplus.netlify.app/` correspondía a otra prueba/cuenta y no debe usarse como dominio operativo actual.

GitHub no es necesario para que el despliegue manual actual funcione. Su rol futuro será versionado y publicación automática. Antes de conectarlo debe definirse una superficie pública mínima para no exponer `input/`, documentación interna, configuraciones o archivos innecesarios del repositorio completo.
...
### Publicación web vigente de TV1–TV6

Todas las salidas comparten el dominio `https://dashoperabp.netlify.app/` y se distinguen por el nombre del HTML:
...
`/` abre `index.html`, actualmente copia de TV1. Netlify no presenta estas rutas como seis proyectos distintos: son seis páginas del mismo proyecto. La actualización actual es manual mediante un nuevo despliegue de `OCU26_WEB`; la automatización queda para Etapa 2.
...
- carpeta separada `OCU26_WEB`;
- despliegue manual mediante Netlify Drop;
- URLs públicas estables `dashoperabp.netlify.app`.

### Planificada — Etapa 2
`Fuente canónica actualizada → validación/Gates → builders → artefactos web → GitHub → Netlify automático → CMS/TVs`

GitHub/Netlify se automatizarán en Etapa 2. GitHub Actions o un mecanismo equivalente se evaluará para regeneración y despliegue automático, pero no está configurado todavía.
...
- conexión inmediata de Netlify al repositorio completo sin controlar archivos públicos;
- automatizar antes de resolver la fuente canónica SharePoint↔repo.
...
### Publicación
Actual: `OCU26_WEB → Netlify Drop → dashoperabp.netlify.app → CMS/TV`.

Etapa 2: `pipeline → superficie pública GitHub → Netlify automático → CMS/TV`.
...
- automatización Etapa 2 GitHub → Netlify;
- repositorio/carpeta pública mínima para despliegue;
- GitHub Actions o mecanismo equivalente de regeneración;
- SharePoint Lists;
- Forms robusto;
...
6. Probar las seis URLs Netlify y luego una URL en TV/CMS.
7. `git status` + `git diff`.
8. commit/push con aprobación.
9. Power BI.
10. SharePoint/Forms Etapa 1.
11. Etapa 2: resolver fuente canónica, crear superficie pública mínima y automatizar GitHub → Netlify.
...
1. conservar `CONTEXTO_MAESTRO.md` como fuente de verdad vigente;
2. reemplazar/actualizar `docs/CM1.md` en el repo cuando corresponda;
3. verificar las seis URLs de `dashoperabp.netlify.app`, especialmente TV5;
4. probar al menos una URL en el equipo real LG/webOS o CMS;
5. continuar el ordenamiento de bases y resolver la fuente canónica SharePoint↔repo;
6. mantener Netlify manual durante esta etapa;
7. implementar la automatización GitHub → Netlify recién en Etapa 2;
8. si se inicia un ajuste visual, hacerlo en la sesión de Claude Code de esa TV con instrucción:
...
13. Se creó `OCU26_WEB` con los seis HTML, seis JSON, los dos GeoJSON y `index.html` como copia de TV1.
14. Se abrió una cuenta/proyecto propio de Netlify mediante autenticación con GitHub.
15. Se desplegó la carpeta mediante Netlify Drop.
16. El nombre provisional `tiny-taffy-7dad45` fue cambiado a `dashoperabp`.
17. Se confirmó que la portada abre `index.html`.
18. Se definieron las seis URLs bajo `https://dashoperabp.netlify.app/`.
19. Se aclaró que Netlify no muestra las páginas internas como proyectos separados: se accede agregando `/tvN.html`.
20. Se decidió no conectar GitHub todavía; el despliegue sigue manual.
21. La automatización GitHub → Netlify quedó ubicada expresamente en la Etapa 2.

Este archivo es la actualización exhaustiva posterior a ese cierre.
...
12. ¿Cómo se resuelve Power BI `dataType`?
13. ¿Cómo funcionará la prueba física webOS/CMS?
14. ¿Cuál será la única fuente canónica al incorporar SharePoint?
15. ¿La automatización Etapa 2 regenerará artefactos en GitHub Actions o mediante otro runner?
16. ¿Se usará una carpeta pública controlada dentro del repo actual o un repositorio de publicación separado?
...
**Netlify Drop:** despliegue manual de una carpeta estática en Netlify.  
**`OCU26_WEB`:** carpeta mínima publicada, separada del repositorio operativo completo.  
**`dashoperabp`:** proyecto Netlify vigente y dominio base de las seis TVs.  
...
### 11–13/8
- trabajo separado de reconstrucción/limpieza YPF Etapa 1;
- ordenamiento de bases y temas de fondo;
- preparación manual de `OCU26_WEB`;
- copia de seis HTML productivos, seis JSON y dos GeoJSON;
- creación de `index.html` como copia de TV1;
- autenticación de Netlify mediante GitHub;
- despliegue manual con Netlify Drop;
- proyecto renombrado a `dashoperabp`;
- dominio público confirmado `https://dashoperabp.netlify.app/`;
- decisión de mantener publicación manual ahora;
- automatización GitHub → Netlify reservada para Etapa 2.
...
24. El dominio operativo vigente es `dashoperabp.netlify.app`.
25. No volver a usar `digitalcore-brandplus.netlify.app` como URL administrada actual.
26. No conectar todavía Netlify al repositorio completo.
27. Mantener despliegue manual en esta etapa.
28. Implementar GitHub → Netlify automático en Etapa 2, después de resolver fuente canónica y superficie pública mínima.
29. No publicar Excel, `input/`, Gates, documentación interna ni configuraciones innecesarias.
...
- [ ] carpeta local `OCU26_WEB`
- [ ] `OCU26_WEB/index.html`
- [ ] registro de URLs `https://dashoperabp.netlify.app/tv1.html` … `/tv6.html`
- [ ] acceso administrativo al proyecto Netlify `dashoperabp`
- [ ] definición futura de carpeta/repositorio público para automatización Etapa 2
...
40. Publicación manual vigente: Netlify `dashoperabp`; próximo: prueba TV/CMS, bases/SharePoint y automatización GitHub → Netlify en Etapa 2.
